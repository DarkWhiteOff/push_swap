...
void    sort_small_pile(int *pilea, int *pileb, int pilesize)
{
    ...
    if (pilesize == 4)
    {
        nb_index = search_nb_index(pilea, pilesize, 4);
        while (pilea[0] != 4)
        {
            if (nb_index <= pilesize / 2)
            {
                ra_rb(pilea, pilesize);
                ft_printf("ra\n");
            }
    ...
    if (pilesize == 5)
    {
        ...
        pb_pa(pilea, pileb, "pb", pilesize, 0);
        ft_printf("pb\n");
        nb_index = search_nb_index(pilea, pilesize - 1, 5);
        while (pilea[0] != 5)
        {
            if (nb_index <= (pilesize - 1) / 2)
            {
                ra_rb(pilea, pilesize - 1);
                ft_printf("ra\n");
            }
            else
            {
                rra_rrb(pilea, pilesize - 1);
                ft_printf("rra\n");
            }
...
int     main(int argc, char *argv[])
{
    ...
    while (i < pilesize)
    {
        pilea[i] = ft_atoi(argv, pilea, pileb, pileacpy, k, pilesize, check);
        i++;
        k++;
    }
    if (check == 1)
